...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
    ...
    const sanityServiceUrls = await client.fetch(groq`*[_type == "securityDivision"]{"slug": slug.current, "lastModified": _updatedAt}`).then((res) => (
        res.map(({ slug, lastModified }: { slug: string, lastModified: Date }) => ({
    ...
    const fallbackServiceUrls = services.map(service => ({
    ...
    const serviceUrls = sanityServiceUrls.length > 0 ? sanityServiceUrls : fallbackServiceUrls;
